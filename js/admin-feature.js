export function createAdminFeature(deps) {
  ...
  function openAdmin(editId = null) {
    ...
    overlay.innerHTML = `
      <div class="modal admin-modal">
        ...
        <div class="admin-actions" style="margin-bottom:15px">
          <button class="btn btn-danger" data-new>+ Nova edição</button><button class="small-btn" data-new-collection>+ Criar coleção</button><button class="small-btn" data-achievements>Títulos</button><button class="small-btn" data-account-plan>Tipo de conta</button><button class="small-btn" data-reader-ads>Propagandas</button><button class="small-btn" data-site-layout>Layout do site</button>
  ...
  function parseEditionCustomLinks(value) {
    ...
    for (let index = 0; index < lines.length; index += 1) {
      ...
      const pipeSeparator = line.indexOf("|");
      const urlMatch = line.match(/https?:\/\/\S+/i);
      let label = "";
      let url = "";
      if (pipeSeparator > 0 && pipeSeparator < line.length - 1) {
        label = line.slice(0, pipeSeparator).trim();
        url = line.slice(pipeSeparator + 1).trim();
      } else if (urlMatch && urlMatch.index > 0) {
        label = line.slice(0, urlMatch.index).trim();
        url = urlMatch[0].trim();
      } else {
        return { links: [], error: `Link extra na linha ${index + 1}: use Nome | URL ou Nome URL.` };
      }
    ...
    overlay.innerHTML = `
      <div class="modal"><div class="section-head"><div><h2>${id ? "Editar edição" : "Nova edição"}</h2><div class="section-subtitle">A capa será extraída da primeira página</div></div><button class="small-btn" data-close>Fechar</button></div>
        ...
        <form id="edit-form"><div class="form-grid">
          ...
          ${isAdminProfile() ? `<div class="field full"><label>Links extras desta edição</label><textarea name="customLinks" rows="4" placeholder="Página oficial | https://...\nEntrevista | https://...">${escapeHTML(customLinksText)}</textarea><small class="format-hint">Somente administradores podem editar. Um por linha. Aceita <b>Nome | URL</b> ou <b>Nome https://...</b>. Os links aparecem como botões apenas no leitor desta edição.</small></div>` : ""}
