(() => {
  ...
  function isSource(value) {
    try { const url = new URL(value); return url.protocol === 'https:' && /(?:^|\.)4shared\.com$/i.test(url.hostname); }
    catch { return false; }
  }
  function proxyUrl(value) {
    if (!isSource(value) || !window.BANCA_SUPABASE_URL) return '';
    const url = new URL(`${window.BANCA_SUPABASE_URL}/functions/v1/fourshared-proxy`);
