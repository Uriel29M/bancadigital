-- Adiciona a subclasse Banca: equipe de moderação com permissão
-- adicional para promover moderadores.
alter table public.profiles drop constraint if exists profiles_plan_check;
alter table public.profiles
  add constraint profiles_plan_check
  check (plan in ('free', 'premium', 'moderator', 'banca', 'admin'));

create or replace function public.is_moderator()
returns boolean language sql stable security definer set search_path = public
as $$ select exists (
  select 1 from public.profiles
  where id = auth.uid() and plan in ('moderator', 'banca', 'admin')
) $$;
...
begin
  ...
  if p_plan = 'banca' and v_actor_plan <> 'admin' then
