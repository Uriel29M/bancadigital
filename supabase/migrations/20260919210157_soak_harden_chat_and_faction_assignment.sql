...
create or replace function public.choose_faction(p_faction_id text default null::text)
...
begin
  ...
  if v_profile.faction_id is not null then
    if p_faction_id is null or p_faction_id = v_profile.faction_id then
      select f.id, f.name, f.color, f.emblem, f.description, v_profile.faction_changed_at
      into v_result_faction_id, v_result_name, v_result_color, v_result_emblem, v_result_description, v_result_changed_at
      from public.factions f where f.id = v_profile.faction_id;
  ...
  if p_faction_id is not null and exists (select 1 from public.factions where id = p_faction_id) then
    if exists (select 1 from public.faction_bans bans where bans.faction_id = p_faction_id and bans.user_id = v_user_id) then
      raise exception 'Você não pode entrar novamente nesta facção';
    end if;
    v_target := p_faction_id;
  else
    perform pg_advisory_xact_lock(hashtext('banca-digital:choose-faction'));
    with counts as (
      select f.id,
        count(m.user_id)::bigint as members,
    ...
    from counts
    where not exists (
      select 1 from public.faction_bans b
      where b.faction_id = counts.id and b.user_id = v_user_id
    )
    order by members asc, activity asc, random()
    limit 1;
  end if;
  ...
  set faction_id = v_target,
      faction_joined_at = coalesce(profile.faction_joined_at, now()),
      faction_changed_at = case when profile.faction_id is null then null else now() end
  ...
  select f.id, f.name, f.color, f.emblem, f.description, p.faction_changed_at
  into v_result_faction_id, v_result_name, v_result_color, v_result_emblem, v_result_description, v_result_changed_at
  from public.factions f
  join public.profiles p on p.faction_id = f.id
  where f.id = v_target and p.id = v_user_id;
