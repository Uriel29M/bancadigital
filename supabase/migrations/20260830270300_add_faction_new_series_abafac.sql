-- Permite que a liderança ative a abafac de séries novas.
...
begin
  ...
  if jsonb_array_length(p_order) > 17 + (select count(*) from public.faction_abafac_images where faction_id = p_faction_id) + (select count(*) from public.faction_abafac_catalogs where faction_id = p_faction_id) + (select count(*) from public.faction_catalogs where faction_id = p_faction_id) then raise exception 'A ordem possui abas inválidas'; end if;
  if exists (select 1 from jsonb_array_elements_text(p_order) item(value) where not (
    item.value in ('stats', 'manifest', 'mural', 'missions', 'mandatory-reads', 'continue-reading', 'recently-added', 'featured-character', 'new-series', 'achievements', 'hall', 'report', 'leadership', 'members')
    ...
    or (item.value ~ '^image:[0-9]+$' and exists (select 1 from public.faction_abafac_images image where image.id = split_part(item.value, ':', 2)::bigint and image.faction_id = p_faction_id))
    or (item.value ~ '^catalog:[0-9]+$' and exists (select 1 from public.faction_abafac_catalogs catalog where catalog.id = split_part(item.value, ':', 2)::bigint and catalog.faction_id = p_faction_id))
    or (item.value ~ '^faction-catalog:[0-9]+$' and exists (select 1 from public.faction_catalogs catalog where catalog.id = split_part(item.value, ':', 2)::bigint and catalog.faction_id = p_faction_id))
