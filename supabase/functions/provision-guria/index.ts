...
Deno.serve(async (request) => {
  ...
  if (!id) {
    ...
    if (created.error && !/already registered|already exists/i.test(created.error.message)) return Response.json({ error: 'create_failed', detail: created.error.message }, { status: 500 });
