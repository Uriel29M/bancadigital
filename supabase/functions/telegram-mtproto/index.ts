...
function databaseUrl(path: string) { return `${required('SUPABASE_URL')}/rest/v1/${path}`; }
...
const handle = createMediaHandler({
  ...
  open: async item => {
    ...
    return { read: (offset: number, limit: number, signal: AbortSignal) => client.downloadChunk({ location: item.fileId, offset, limit, abortSignal: signal, maxRetryCount: 2, floodSleepThreshold: 0 }), close: async () => {} };
  },
});
Deno.serve(handle);