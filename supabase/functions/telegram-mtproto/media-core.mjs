export class MediaError extends Error {
  constructor(message, status = 502, code = 'media_error') { super(message); this.status = status; this.code = code; }
}
export const cors = {
...
export function parseRange(value, size) {
  ...
  let start, end;
  if (!match[1]) { const suffix = Number(match[2]); if (!Number.isSafeInteger(suffix) || suffix < 1) return false; start = Math.max(0, size - suffix); end = size - 1; }
...
function json(body, status = 200, extra = {}) { return new Response(JSON.stringify(body), { status, headers: { ...cors, 'Content-Type': 'application/json; charset=utf-8', ...extra } }); }
export function createMediaHandler({ lookup, open, health = async () => ({ ok: true }), maxConcurrent = 8, chunkSize = 262144 }) {
  ...
  return async function handle(request) {
    ...
    try {
      ...
      const item = await lookup(itemId); if (!item) throw new MediaError('Edição indisponível.', 404);
      ...
      const start = range ? range.start : 0; const end = range ? range.end : size - 1;
      ...
      active++; let source, released = false;
      const release = async () => { if (released) return; released = true; active--; try { await source?.close?.(); } catch {} };
      try {
        source = await open(item, request.signal); let offset = start;
        ...
        const stream = new ReadableStream({
          async pull(controller) {
            try {
              ...
              const bytes = first || await source.read(offset, Math.min(chunkSize, end - offset + 1), request.signal); first = null;
              ...
              offset += bytes.length; controller.enqueue(bytes); if (offset > end) { controller.close(); await release(); }
      ...
      return json({ error: error instanceof MediaError ? error.message : 'Falha ao acessar o arquivo.', code: error instanceof MediaError ? error.code : 'media_error' }, status);
    }
  };
}