...
revoke execute on function public.create_notification(uuid, text, text, text, uuid, text, jsonb) from public, anon, authenticated;
...
-- Profile write security (keep aligned with secure_profile_writes migration).
...
-- Public profile boundary and server-side username login.
...
-- RPC authorization audit and explicit grants.
