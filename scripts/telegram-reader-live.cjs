...
edit(app,'    const sourceCandidates = readerSourceCandidates(item);','    const telegramDownloadController = new AbortController();\n    const sourceCandidates = readerSourceCandidates(item);');
edit(app,'    const prefetchedBuffer = isTelegramMediaUrl(resolvedUrl) ? null : readerFilePrefetches.get(resolvedUrl) || null;','    const prefetchedBuffer = isTelegramMediaUrl(resolvedUrl) ? null : readerFilePrefetches.get(resolvedUrl) || null;');
...
edit(app,'    if (!isMega) {\n      // Tenta uma única transferência;','    if (!isMega) {\n      // Tenta uma única transferência;');
edit(app,'      let buffer = await waitForPrefetchedBuffer(prefetchedBuffer);','      let buffer = await waitForPrefetchedBuffer(prefetchedBuffer);');
